...
const char* MyErrorToString(HRESULT error);
...
static BOOL CreateScene(AppInfo* info)
{
	...
	goto generic_error;
    rval = builder->Load((LPVOID)"camera.x", NULL, D3DRMLOAD_FROMFILE, NULL, NULL);
    if (rval != D3DRM_OK) {
    ...
    if (FAILED(lpD3DRM->CreateMeshBuilder(&builder)))
	goto generic_error;
    rval = builder->Load((LPVOID)"mslogo.x", NULL, D3DRMLOAD_FROMFILE,
	loadTextures, NULL);
    if (rval != D3DRM_OK) {
...
CreateLight(WPARAM wparam, AppInfo* info)
{
	...
	rval = builder->Load((LPVOID)"camera.x", NULL, D3DRMLOAD_FROMFILE,
			     NULL, NULL);
	if (rval != D3DRM_OK) {
	    Msg("Failed to load camera.x.\n%s", MyErrorToString(rval));
	    goto ret_with_error;
	...
	rval = builder->Load((LPVOID)"sphere2.x", NULL, D3DRMLOAD_FROMFILE,
			     NULL, NULL);
	if (rval != D3DRM_OK) {
