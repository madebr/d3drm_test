				 ...
				 LPDIRECT3DRMFRAME3 )
{
    ...
    if( FAILED( pD3DRM->CreateMeshBuilder( &pBuilder)))
	    goto generic_error;
    if( FAILED( pBuilder->Load( (LPVOID)"sphere3.x", NULL, D3DRMLOAD_FROMFILE, NULL,
		                        NULL)))
	{
    ...
    if( FAILED( pD3DRM->CreateMeshBuilder( &pBuilder ) ) )
	    goto generic_error;
    if( FAILED( pBuilder->Load( (LPVOID)"tpot2.x", NULL, D3DRMLOAD_FROMFILE, NULL,
		                        NULL ) ) )
	    goto generic_error;
					   ...
					   BOOL* pbConstRenderQuality, CHAR** pstrName )
{
    ...
    (*pstrName) = (CHAR *)"Texture Mapping III Direct3DRM Example";
}
