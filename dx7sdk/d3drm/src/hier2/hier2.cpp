	             ...
	             LPDIRECT3DRMFRAME3 scene, LPDIRECT3DRMFRAME3 camera )
{
    ...
    if( FAILED( pD3DRM->CreateMeshBuilder( &cube2_builder ) ) )
		goto generic_error;
    if( FAILED( torus_builder->Load( (void *)"torus.x", NULL, D3DRMLOAD_FROMFILE,
		                             NULL, NULL ) ) )
    {
		MSG("Failed to load torus.x.\n");
		goto ret_with_error;
    }
    if( FAILED( sphere_builder->Load( (void *)"sphere4.x", NULL, D3DRMLOAD_FROMFILE,
		                              NULL, NULL ) ) )
	{
        MSG("Failed to load sphere4.x.\n");
		goto ret_with_error;
    }
    if( FAILED( cube1_builder->Load( (void *)"cube.x", NULL, D3DRMLOAD_FROMFILE,
		                             NULL, NULL ) ) )
	{
        MSG("Failed to load cube.x.\n");
		goto ret_with_error;
    }
    if( FAILED( cube2_builder->Load( (void *)"cube.x", NULL, D3DRMLOAD_FROMFILE,
		                             NULL, NULL ) ) )
	{
        MSG("Failed to load cube.x.\n");
		goto ret_with_error;
					   ...
					   BOOL* pbConstRenderQuality, CHAR** pstrName )
{
    ...
    (*pstrName) = (CHAR *)"Frame Hierarchy II D3D RM Example";
}
