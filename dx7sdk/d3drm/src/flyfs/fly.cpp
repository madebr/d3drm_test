	             ...
	             LPDIRECT3DRMFRAME3 pScene, LPDIRECT3DRMFRAME3 pCamera )
{
    ...
    if( FAILED( pD3DRM->CreateMeshBuilder( &smoke_builder ) ) )
		goto generic_error;
    if( FAILED( smoke_builder->Load( (LPVOID)"sphere0.x", NULL, D3DRMLOAD_FROMFILE,
		                             NULL, NULL ) ) )
	{
    ...
    if( FAILED( pD3DRM->CreateMeshBuilder( &mesh_builder ) ) )
		goto generic_error;
    if( FAILED( mesh_builder->Load( (LPVOID)"land4.x", NULL, D3DRMLOAD_FROMFILE,
		                            NULL, NULL ) ) )
		goto generic_error;
	...
	if( FAILED( plane_builder->Load( (LPVOID)"dropship.x", NULL, D3DRMLOAD_FROMFILE,
		                             NULL, NULL ) ) )
	{
...
VOID OverrideDefaults( BOOL* pbNoTextures, BOOL* pbResizingDisabled,
					   BOOL* pbConstRenderQuality, const CHAR** pstrName )
{
	(*pbNoTextures) = TRUE;
