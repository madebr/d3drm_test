...
LPCSTR GetDirectPlayErrStr( HRESULT hr );
                          ...
                          DWORD dwMsgSize, DPID idFrom, DPID idTo )
{
    ...
    switch( pMsg->dwType )
    {
        ...
        case DPSYS_DELETEPLAYERFROMGROUP:
        {
            ...
            LPCSTR  strPlayerName, strGroupName;
            DPNAME* pPlayerName;
            DPNAME* pGroupName;
            LPCSTR  strDisplayFormat;
            HRESULT hr;
        ...
        case DPSYS_DELETEGROUPFROMGROUP:
        {
            ...
            LPCSTR  strParentGroupName, strGroupName;
        ...
        case DPSYS_SESSIONLOST:
        {
            ...
            LPCSTR strDisplayFormat = "Session lost.\r\n";
        ...
        case DPSYS_HOST:
        {
            ...
            LPCSTR      strDisplayFormat = "You have become the host\r\n";
        ...
        case DPSYS_SETGROUPOWNER:
        {
            ...
            LPCSTR  strGroupName = NULL;
            LPCSTR  strNewOwnerName = NULL;
            LPCSTR  strOldOwnerName = NULL;
            DPNAME* pGroupName = NULL;
            DPNAME* pNewOwnerName = NULL;
            DPNAME* pOldOwnerName = NULL;
            LPCSTR  strDisplayFormat = "Owner changed for group \"%s\" to \"%s\" from \"%s\"\r\n";
            HRESULT hr;
...
HRESULT InitializeLobby( HWND hWnd, DPLAYINFO* pDPInfo )
{
    ...
    if( pDPInfo->bIsHost )
    {
        ...
        name.lpszShortNameA = (LPSTR)"Golf Shack";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;

        name.lpszShortNameA = (LPSTR)"Monster Truck Rally";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;

        name.lpszShortNameA = (LPSTR)"Club Hellbender";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;
    }
...
// Desc:
//-----------------------------------------------------------------------------
VOID LogString( LPCSTR strDisplayFormat, LPCSTR strData )
{
    // Allocate space for string
                       ...
                       LPSTR* pstrStr )
{
    ...
    LPSTR   strStr = NULL;
    LPSTR   strPlayerName;
    LPSTR   strDisplayFormat = (LPSTR)"%s>\t%s\r\n";
    HRESULT hr;
...
// Desc:
//-----------------------------------------------------------------------------
VOID ErrorBox( LPCSTR strError, HRESULT hr )
{
    CHAR strStr[MAXSTRLEN];
