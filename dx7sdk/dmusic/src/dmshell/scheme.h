...
struct SCHEME
{
	PCWSTR FriendlySchemeName;				//Human readable name for the menu
	PCWSTR StyleName;						//Filename
	PCWSTR DLSName;							//Filename (blank if the style uses GM.DLS)
	PCWSTR ChordMapName;					//Filename
	PCWSTR TemplateNames[NUM_TEMPLATES];	//Filename
	PCWSTR MotifNames[DME_MAXEVENTNUM];		//Motif name contained in the style (referenced above)
};
