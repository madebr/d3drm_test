...
CDMPlayer::CDMPlayer(HWND hWnd)
{
    ...
    if ( FAILED(hr) )
    {
	    hr = m_pLoader->SetSearchDirectory(GUID_DirectMusicAllTypes, (WCHAR *)L".", FALSE );
    }
...
BOOL CDMPlayer::LoadStyle(const WCHAR *pwzFileName)
{
...
BOOL CDMPlayer::LoadChordMap(const WCHAR *pwzFileName)
{
	DMUS_OBJECTDESC ObjectDescript;
	ObjectDescript.dwSize = sizeof(DMUS_OBJECTDESC);
...
BOOL CDMPlayer::LoadTemplate(DWORD dwIndex, const WCHAR *pwzName)
{
...
BOOL CDMPlayer::LoadDLS(const WCHAR *sFileName)
{
	...
	for ( x = 0; x < DME_MAXEVENTNUM ; x++)
	{
		if ( Schemes[wSchIndx].MotifNames[x] )
			if (wcscmp(Schemes[wSchIndx].MotifNames[x], L"") != 0)
				m_pStyle->GetMotif((WCHAR *)Schemes[wSchIndx].MotifNames[x], &m_pMotifs[x]);
	}
...
void CDMPlayer::NullMediaPointers()
{
	...
	for (int x = 0; x < DME_MAXEVENTRANGE; x++)
