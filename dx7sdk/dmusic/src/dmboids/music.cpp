...
BOOL BoidMusic::LoadTemplate(DWORD dwIndex, const WCHAR * pwzName)
...
BOOL BoidMusic::GetMotif(DWORD dwIndex, const WCHAR * pszName)
...

{
	if (m_pStyle)
	{
		if (SUCCEEDED(m_pStyle->GetMotif((WCHAR *)pszName, &m_pMotifSegments[dwIndex])))
        {
            return TRUE;
        }
	}
