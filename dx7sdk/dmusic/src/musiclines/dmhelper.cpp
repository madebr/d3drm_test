...
IDirectMusicLoader* CreateLoader(void)
{
	...
	else
	{
        ...
        if (FAILED(hr))
        {
		    hr = pLoader->SetSearchDirectory( GUID_DirectMusicAllTypes, (WCHAR *)L".", FALSE );
        }
...
IDirectMusicSegment* CreateSegmentFromFile(IDirectMusicLoader* pLoader, const WCHAR* wszFileName)
{
...
IDirectMusicStyle* GetStyle(IDirectMusicLoader* pLoader, const WCHAR* wszStyle)
{
	DMUS_OBJECTDESC ObjDesc; // Object descriptor for pLoader->GetObject()
...
IDirectMusicChordMap* GetChordMap(IDirectMusicLoader* pLoader, const WCHAR* wszChordMap)
{
	DMUS_OBJECTDESC ObjDesc; // Object descriptor for pLoader->GetObject()
							 ...
							 const WCHAR* wszMotif)
{
	...
	if( pStyle )
	{
		pStyle->GetMotif((WCHAR *)wszMotif, &pSegment);
		pStyle->Release();
	}
	return pSegment;
}
