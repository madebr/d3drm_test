...
public:
    HRESULT    GetMeshVertices( const TCHAR* strName, D3DVERTEX** ppVertices,
                                DWORD* pdwNumVertices );
    HRESULT    GetMeshIndices( const TCHAR* strName, WORD** ppIndices,
                               DWORD* pdwNumIndices );

    CD3DFileObject* FindObject( const TCHAR* strName );
    ...
    HRESULT Load( const TCHAR* strFilename );
