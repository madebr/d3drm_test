...
//-----------------------------------------------------------------------------
HRESULT CD3DFile::Load( const TCHAR* strFilename )
{
    HRESULT                 hr;
    ...
    hr = pDXFile->CreateEnumObject( (LPVOID) strFilename, DXFILELOAD_FROMFILE, &pEnumObj );
    ...
    m_pRoot = new CD3DFileObject( (TCHAR *) _T("D3DFile_Root") );
...
//-----------------------------------------------------------------------------
HRESULT CD3DFile::GetMeshVertices( const TCHAR* strName, D3DVERTEX** ppVertices,
...
//       object, and retrieves the vertices for the specified mesh.
//-----------------------------------------------------------------------------
HRESULT CD3DFile::GetMeshIndices( const TCHAR* strName, WORD** ppIndices,
                                  DWORD* pdwNumIndices )
{
    CD3DFileObject* pObject = FindObject( strName );
    if( pObject )
...
//-----------------------------------------------------------------------------
CD3DFileObject* CD3DFile::FindObject( const TCHAR* strName )
{
    if( NULL == strName )
        return m_pRoot;
    ...
    FINDMESHRECORD data = { (TCHAR *)strName, NULL };
